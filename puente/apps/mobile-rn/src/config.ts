...
/** myeyescantalk Mac (POST /command). */
export const COMMAND_BASE_URL =
  process.env.EXPO_PUBLIC_COMMAND_BASE_URL ||
  WORKER_BASE_URL.replace(":8787", ":8788");

export const COMMAND_TOKEN = process.env.EXPO_PUBLIC_COMMAND_TOKEN ?? "";

/** eyesstreelighttalk ws_bridge (cruce peatonal). */
export const CROSSING_WS_URL =
  process.env.EXPO_PUBLIC_CROSSING_WS_URL ||
  WORKER_BASE_URL.replace("http://", "ws://").replace(":8787", ":8765");
